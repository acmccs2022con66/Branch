...
static PyObject* Encrypt(PyObject *self , PyObject *args)
{
    ...
    unsigned char k_id1[EVP_MAX_MD_SIZE]={0}; unsigned int k_id1_len;
    unsigned char k_id2[EVP_MAX_MD_SIZE]={0}; unsigned int k_id2_len;


    unsigned char ct_1[EVP_MAX_MD_SIZE] = {0}; unsigned int ct_1_len;
    unsigned char ct_2[EVP_MAX_MD_SIZE] = {0}; unsigned int ct_2_len;
    ...
    unsigned int keyword_cnt,fileid_cnt;

    if(!PyArg_ParseTuple(args , "sis" , &keyword, &keyword_cnt, &fileid))
    {
      return NULL;
    }
	...
	t_start = clock();
		...
		//ct2
		HMAC(EVP_sha256(),k_w2 , k_w2_len , ct_1 , ct_1_len , ct_2 , &ct_2_len);

		memcpy(fileid_bytes , fileid , strlen(fileid));

		_xor(ct_2,fileid_bytes,ct_2_len, ct_2);
